...
def build_po_to_dispatch_workflow(org_id: str, source: str, raw_input: dict) -> tuple[WorkflowEngine, WorkflowContext]:
    ...
    async def step_generate_documents(ctx: WorkflowContext) -> StepResult:
        ...
        # apply WhatsApp confidence floor so it doesn't drop below auto-verification threshold unnecessarily
        if ctx.raw_input.get("buyer_whatsapp"):
            ctx.overall_confidence = max(ctx.overall_confidence, _cfg.CONFIDENCE_FLOOR_WHATSAPP)
